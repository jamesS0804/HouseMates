        ...
        <div className={`w-screen fixed bottom-0 left-0 p-0 grid grid-cols-4 gap-0 text-white ${userType === 'Homeowner' ? 'bg-primary' : 'bg-secondary'}`}>
            {
                navigationOptions.map((option)=>{
                    return(
                        <Link key={option.title} type="button" to={option.path} className={`grow py-3 px-5 flex flex-col justify-center items-center ${selectedOption === option.title.toLocaleLowerCase() ? userType === 'Homeowner' ? 'bg-primary translate-y-[-1rem] rounded-t-[5rem]' : 'bg-secondary translate-y-[-1rem] rounded-t-[5rem]' : ''}`}>
